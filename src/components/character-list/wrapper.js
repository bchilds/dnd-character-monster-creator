...
const CharacterListWrapper = ({ children }) => {
  ...
  const setCharacterById = useCallback(characterToSet => {
    const { _id } = characterToSet;
    const foundCharacterIndex = characters.indexOf(char => char._id === _id);
  ...
  const setAllCharacters = useCallback(newCharacters => {
    setCharacters(newCharacters);
  }, [setCharacters]);

  const fetchAndSetAllCharacters = useEffect(() => {
